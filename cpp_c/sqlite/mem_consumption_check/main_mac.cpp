...
#define PARAM_NUM 5
static int converted_param[PARAM_NUM] = {
	0,  // through id
	10, // mod 10
	10, // mod 10
	10, // mod 10
...
static int value_gen(int sql_id, int param_id) //  0
{
	...
	if (converted_param[param_id] == 0) {
		// through
		return sql_id;
	} else if (converted_param[param_id] > 0) {
		// positive value => mod
		return sql_id % converted_param[param_id];
	}
	// negative => div
	return sql_id / (0 - converted_param[param_id]);
}
static int set_conveted_param(const char* arg, int param_id)
{
	...
	if (strcmp(arg, "0") == 0
		...
		|| strcmp(arg, "sql_id") == 0) {
			...
			converted_param[param_id] = 0;
			return 0;
	} else if (arg[0] == 'm'
		&& arg[1] == 'o'
		&& arg[2] == 'd') {
			converted_param[param_id] = atoi(&arg[3]);
			printf("\tset_converted_param(): mod %d for: %d\n", converted_param[param_id], param_id);
			return 0;
	} else if (arg[0] == 'd'
		&& arg[1] == 'i'
		&& arg[2] == 'v') {
			converted_param[param_id] = 0 - atoi(&arg[3]); // negative val
			printf("\tset_converted_param(): div %d for: %d\n", 0 - converted_param[param_id], param_id);
			return 0;
	}
...
//       -1..5 PARAM_VAL: "sql_id", "mod?", "div?" (? can be any number).
...
int check_args(int argc, char** argv)
{
	...
	while (argc > arg_index) {
		...
		if (argv[arg_index][1] == 'v' || argv[arg_index][1] == 'V') {
			...
			VERBOSE_PRINTF("set soft heap limit: %lld\n", soft_heap_limit);
		...
		} else if (argv[arg_index][1] >= '1' && argv[arg_index][1] <= '5') {
...
//		RemoveFile(db_name);
	...
	if ( soft_heap_limit < 0 ) {
		...
		printf("sqlite3_soft_heap_limit64(): prev: %lld (byte) is set to: %lld (byte).\n", soft_heap_limit, sqlite3_soft_heap_limit64(-1));
	}
	...
	do {
		...
		printf("[%d] th transaction is end. operated sql num: %d, time: %llu (us)", trans_count, sql_count, end_time - start_time);
		...
		printf("\t: sqlite3_memory_highwater(true): %lld\n", highwater);
	} while (sql_count < total_sql_num);
	printf("operated sql count: %d, transaction count: %d\n", sql_count, trans_count);
	printf("total elapsed time: %llu (us)\n", total_time);
	if (trans_count != 0) {
		printf("maximum memory highwater: %lld, average high water / transaction: %llu\n", max_highwater, total_highwater / trans_count);
	}
